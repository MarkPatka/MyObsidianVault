...
1.  **Нарушение принципа Open/Closed (OCP):** Если вы захотите добавить новый статус `ReadyForPickup`, вам придется **прочесать весь код** и найти все места, где есть `if` или `switch` по `OrderStatus`, и добавить туда новую ветку. 
